import type { TranslationKey } from "./az";

export const ru: Record<TranslationKey, string> = {
  "app.full": "Помощник абитуриента БГУ",

  "sidebar.new": "Новый чат",
  "sidebar.sections": "Разделы",
  "sidebar.collapse": "Свернуть панель",
  "sidebar.expand": "Развернуть панель",
  "sidebar.open": "Открыть панель",
  "sidebar.close": "Закрыть панель",

  "topbar.title": "Помощник абитуриента",
  "topbar.restart": "Начать заново",

  "chat.greeting":
    "Здравствуйте! 👋 Добро пожаловать в информационный помощник Бакинского государственного университета для абитуриентов на основе искусственного интеллекта. Мы готовы помочь со всеми вопросами об университете, выборе специальности, правилах приёма и студенческой жизни.",
  "chat.loading": "Готовлю ответ…",
  "chat.scrollToBottom": "Вниз",
  "chat.error": "Произошла ошибка соединения.",
  "chat.retry": "Повторить",
  "chat.copy": "Копировать",
  "chat.copied": "Скопировано",
  "chat.openLink": "Открыть ссылку",
  "chat.imageOpen": "Открыть изображение в полном размере",
  "chat.imageFailed": "Не удалось загрузить изображение",

  "menu.searchPlaceholder": "Поиск по списку…",
  "menu.noMatches": "Совпадений не найдено.",
  "menu.showAll": "Показать ещё {count}",
  "menu.showLess": "Свернуть список",

  "operator.contact": "Связаться с оператором",
  "operator.askPhone":
    "Для связи с оператором укажите ваш контактный номер (например, +994501234567).",
  "operator.askMessage": "Теперь напишите ваш вопрос — оператор свяжется с вами.",
  "operator.phonePlaceholder": "+994 50 123 45 67",
  "operator.messagePlaceholder": "Напишите ваш вопрос…",
  "operator.send": "Отправить",
  "operator.next": "Продолжить",
  "operator.cancel": "Отмена",
  "operator.cancelled": "Запрос к оператору отменён.",
  "operator.sending": "Отправка…",
  "operator.phoneInvalid":
    "Укажите контактный номер в правильном формате (например, +994501234567).",
  "operator.submitted":
    "Ваш запрос принят — оператор скоро свяжется с вами.",
  "operator.submitFailed":
    "Не удалось записать ваш запрос. Пожалуйста, попробуйте немного позже.",
  "operator.networkError":
    "Произошла ошибка соединения. Пожалуйста, попробуйте ещё раз.",

  "nav.home": "Главное меню",
  "nav.back": "Назад",
  "nav.prev": "Предыдущая",
  "nav.next": "Следующая",
  "nav.page": "{page} / {total}",

  "common.close": "Закрыть",
  "common.settings": "Настройки",
  "common.help": "Помощь",
  "common.theme": "Тема",

  "settings.title": "Настройки",
  "settings.tab.general": "Общие",
  "settings.tab.chat": "Чат",
  "settings.tab.about": "О приложении",
  "settings.theme": "Тема",
  "settings.theme.light": "Светлая",
  "settings.theme.dark": "Тёмная",
  "settings.theme.system": "Системная",
  "settings.fontSize": "Размер шрифта",
  "settings.fontSize.small": "Мелкий",
  "settings.fontSize.medium": "Средний",
  "settings.fontSize.large": "Крупный",
  "settings.about.description":
    "Добро пожаловать в службу информации для абитуриентов Бакинского государственного университета.",

  "user.name": "Абитуриент",
  "user.role": "Гость",
  "user.menu": "Меню пользователя",

  "offline.banner":
    "Нет подключения к интернету — продолжите после восстановления связи.",

  "aria.chatLog": "Сообщения чата",
  "aria.sidebar": "Панель навигации",
  "aria.main": "Основной контент",
  "aria.menuOptions": "Опции меню",
  "aria.menuNav": "Навигация по меню",
};
